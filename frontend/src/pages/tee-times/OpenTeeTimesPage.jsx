        ...
        <div className="space-y-4">
          ...
          {teeTimes.map(tt => (
            <div key={tt.id} className="bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700 p-5">
              ...
              {tt.notes && <p className="mt-3 text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-750 rounded-lg p-3">{tt.notes}</p>}
              ...
              {tt.interests && tt.interests.length > 0 && (
                <div className="mt-3 pt-3 border-t dark:border-gray-700">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {tt.interests.map(i => (
                      <span key={i.id} className={`text-xs px-2 py-1 rounded-full ${i.status === 'ACCEPTED' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400'}`}>
            ...
            <div key={tt.id} className="bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700 p-5">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-sm text-gray-500">{formatDate(tt.teeTime)} at {formatTime(tt.teeTime)}</p>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`text-xs px-2 py-1 rounded font-medium ${tt.status === 'OPEN' ? 'bg-green-100 text-green-700' : tt.status === 'FILLED' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700'}`}>
                    {tt.status}
                  </span>
                  {tt.status === 'OPEN' && (
              ...
              {tt.interests && tt.interests.length > 0 && (
                <div className="border-t dark:border-gray-700 pt-3 mt-3">
                  ...
                  <div className="space-y-2">
                    {tt.interests.map(i => (
                      <div key={i.id} className="flex items-center justify-between bg-gray-50 dark:bg-gray-750 rounded-lg px-3 py-2">
                        <div className="flex items-center gap-2">
